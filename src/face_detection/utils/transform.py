...
from src.face_detection.utils.box_utils import matrix_iof
...
def resize_subtract_mean(image, target_size, bgr_mean):
    ...
    resized_image = cv2.resize(
        image, (target_size, target_size), interpolation=interpolation_method
    )
...
class Augmentation:
    ...
    def __call__(self, image, targets):
        ...
        image, boxes, labels, landmarks, requires_padding = _crop(
            image, boxes, labels, landmarks, self.image_size
        )
